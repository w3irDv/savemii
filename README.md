...
This task allows you to wipe  savedata belonging to teh selecte list of titles.

1. Select wich data to wipe: `No Profile Users`, `All users`, `From: select source user / To: select target user`. `common save: Yes/No`. You can also choose if you want to perform a full backup (recommended).
2. The list of all titles that are installed, that match  the savedata criteria choosen in the previous step will appear. You can select / deselect which titles to copy with `Y` `<-` and `->`, or select/deselect all at once with `+` and `-`. 
...
5. The list of all titles will appear again, now showing the wipestatus. You can try to select failed titles and wipe them again to see what the error is. Succesfully wiped titles will be skipped.
...
This task allows you to move / copy  savedata from one profile to a different profile for all titles already installed in the Wii U.
...
2. The list of all titles that are installed, that match  the savedata criteria choosen in the previous step will appear. You can select / deselect which titles to move / copy with `Y` `<-` and `->`, or select/deselect all at once with `+` and `-`. 
...
3. The list of all titles that are installed on both NAND and USB, and  that match  the savedata criteria choosen in the previous step will appear. You can select / deselect which titles to copy with `Y` `<-` and `->`, or select/deselect all at once with `+` and `-`. 
...
6. The list of all titles will appear again, now showing the copy status. You can try to select failed titles and restore them again to see what the error is. Succesfully copied titles will be skipped.
