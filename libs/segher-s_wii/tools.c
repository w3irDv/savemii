...
void md5(u8 *data, u32 len, u8 *hash) {
    ...
    mbedtls_md5_ret((unsigned char *) data, len, hash);
}
...
int check_ec(u8 *ng, u8 *ap, u8 *sig, u8 *sig_hash) {
    ...
    sha(ap + 0x80, 0x100, ap_hash);

    ap_Q = ap + 0x0108;
    sig_R = sig;
    sig_S = sig + 30;

    return check_ecdsa(ng_Q, ap_R, ap_S, ap_hash) && check_ecdsa(ap_Q, sig_R, sig_S, sig_hash);
}
...
void do_yaz0(u8 *in, [[maybe_unused]] u32 in_size, u8 *out, u32 out_size) {
    u32 nout;
    u8 bits;
    u32 nbits;
    u32 n, d, i;
