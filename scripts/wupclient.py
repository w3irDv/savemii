...
# qwii feb26: add (perhaps) mkquota & chown
import errno    
import socket
import os
import sys
...
def copy_dword(buffer, w, offset):
...
class wupclient:
    ...
    # quota
    def FSA_MakeQuota(self, handle, path, mode, size):
        inbuffer = buffer(0x520)
        copy_string(inbuffer, path, 0x0004)
        copy_word(inbuffer, mode, 0x0284)
        copy_dword(inbuffer, size, 0x0288)
        (ret, _) = self.ioctl(handle, 0x1D, inbuffer, 0x297)
        return ret

    # owner
    def FSA_ChangeOwner(self, handle, path, owner, group):
        unk = 0x0
        inbuffer = buffer(0x520)
        copy_string(inbuffer, path, 0x0004)
        copy_word(inbuffer, unk, 0x0284)
        copy_word(inbuffer, owner, 0x0288)
        copy_word(inbuffer, unk, 0x028C)
        copy_word(inbuffer, group, 0x0290)
        (ret, _) = self.ioctl(handle, 0x70, inbuffer, 0x29B)
        return ret
    ...
    # quota
    def mkquota(self, filename, flags, size):
        fsa_handle = self.get_fsa_handle()
        if filename[0] != "/":
            filename = self.cwd + "/" + filename
        ret = w.FSA_MakeQuota(fsa_handle, filename, flags, size)
        print("quota returned : " + hex(ret))
    ...
    def chown(self, filename, owner, group):
        ...
        if filename[0] != "/":
            filename = self.cwd + "/" + filename
        ret = w.FSA_ChangeOwner(fsa_handle, filename, owner, group)
        print("chown returned : " + hex(ret))
...
#silent version
    ...
    def stat(self, filename):
        ...
        if ret != 0x0:
            ...
            print("quota: " + hex(stats[7]) + hex(stats[8]))
        ret = self.FSA_CloseFile(fsa_handle, file_handle)
