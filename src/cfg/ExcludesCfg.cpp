...
bool ExcludesCfg::mkJsonCfg() {
    ...
    if (config == nullptr) {
        Console::showMessage(ERROR_CONFIRM, _("Error creating JSON object: %s"), cfg.c_str());
        return false;
    }
    ...
    if (excludes == nullptr) {
        json_decref(config);
        Console::showMessage(ERROR_CONFIRM, _("Error creating JSON array: %s"), cfg.c_str());
        return false;
    }
    ...
    for (const titleID &titleID : titlesID) {
        ...
        if (element == nullptr) {
            ...
            Console::showMessage(ERROR_CONFIRM, _("Error creating JSON object: %s"), titleKO);
    ...
    if (configString == nullptr) {
        Console::showMessage(ERROR_CONFIRM, _("Error dumping JSON object: %s"), cfg.c_str());
        return false;
    }
...
bool ExcludesCfg::parseJsonCfg() {
    ...
    if (!root) {
        Console::showMessage(ERROR_CONFIRM, _("Error decoding JSON file\n %s\nin line %d:\n\n%s"), cfgFile.c_str(), error.line, error.text);
        return false;
    }

    json_t *excludes = json_object_get(root, cfg.c_str());
    if (excludes == nullptr) {
        Console::showMessage(ERROR_CONFIRM, _("Error: unexpected format (%s not an array)"), cfg.c_str());
        json_decref(root);
        return false;
    }
