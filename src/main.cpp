...
static void disclaimer() {
    consolePrintPosAligned(13, 0, 1,"SaveMii v%u.%u.%u%c-660-permissions-beta", VERSION_MAJOR, VERSION_MINOR, VERSION_MICRO, VERSION_FIX);
...
int main() {
    ...
    while (State::AppRunning()) {
        ...
        if (DrawUtils::getRedraw()) {
            ...
            consolePrintPos(0, 0, "SaveMii v%u.%u.%u%c-p", VERSION_MAJOR, VERSION_MINOR, VERSION_MICRO, VERSION_FIX);
            consolePrintPos(0, 1, "----------------------------------------------------------------------------");
