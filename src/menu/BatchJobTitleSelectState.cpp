...
void BatchJobTitleSelectState::executeBatchProcess() {
    ...
    for (int i = 0; i < titlesCount; i++) {
        ...
        switch (jobType) {
            case RESTORE:
                retCode = restoreSavedata(&this->titles[i], 0, source_user, wiiu_user, effectiveCommon, this->titles[i].currentDataSource.backupFormat, NON_INTERACTIVE); //always from slot 0
                break;
            case PROFILE_TO_PROFILE:
