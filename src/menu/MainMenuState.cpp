...
ApplicationState::eSubState MainMenuState::update(Input *input) {
    if (this->state == STATE_MAIN_MENU) {
        ...
        if (input->get(ButtonState::TRIGGER, Button::Y) || input->get(ButtonState::REPEAT, Button::Y)) {
            ...
            /*
            Console::showMessage(OK_CONFIRM, "easy peasy");
            Console::showMessage(OK_CONFIRM, "easy peasy\nsecondline");
            Console::showMessage(OK_CONFIRM, "En un lugar de la mancha, de cuyo nombre no quiero acordarme, vivia no hace mucho \nha un locuelo bergantin, con cien cañones por bada, viento en popa a toda vela unficheromuylrgoquedeeriarepartirseenvariaslineassiesqetodoestoestafuncionando\n\ny el final");
            
            Console::showMessage(OK_CONFIRM, LanguageUtils::gettext("Error creating/removing (test) target folder\n\n%s\n\n%s\n\nMove not tried!"), path.c_str(), strerror(errno));
            Console::showMessage(ERROR_SHOW, LanguageUtils::gettext("Error creating/removing (test) target folder\n\n%s\n\n%s\n\nMove not tried!"), path.c_str(), strerror(errno));
            
            Console::showMessage(ERROR_SHOW, LanguageUtils::gettext("Failed to delete (legacy) folder\n\n%s\n\n%s"),path.c_str(), strerror(errno));
            */
            ...
            Console::showMessage(OK_CONFIRM, LanguageUtils::gettext("Error creating/removing (test) target folder\n\n%s\n\n%s\n\nMove not tried!"), path.c_str(), strerror(errno));
            Console::showMessage(WARNING_CONFIRM, LanguageUtils::gettext("Failed to delete (legacy) folder\n\n%s\n\n%s"),path.c_str(), strerror(errno));
            Console::showMessage(WARNING_SHOW, LanguageUtils::gettext("Failed to delete (legacy) folder\n\n%s\n\n%s"),path.c_str(), strerror(errno));
            ...
            std::string message = StringUtils::stringFormat(choices, 2345612, 232345, "smash bros is a small tittle, this is better", "Wii U Titles");
            Console::showMessage(OK_CONFIRM, message.c_str());

            const char *choices2 = LanguageUtils::gettext("SaveMii is now using a new name format for savedata folders. Instead of using hex values, folders will be named after the title name, so for this title, folder '%08x%08x' would become '%s', easier to locate in the SD.\n\nDo you want to rename already created backup folders?\n\n\ue000  Yes, but only for this title\n\ue045  Yes, please migrate all %s\n\ue001  Not this time\n\ue002  Not in this session\n\n\n");
