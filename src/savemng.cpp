...
static int stepTitles = 0;
static int stepProfiles = 0;
...
int removeSavedataForNonExistentProfiles(Title* titles, int titlesCount ,eNEProfileWipeMode nEPWipeMode, int totalTitlesToFix, int totalProfilesToFix) {
    int profilesWiped;
    int profileWipeAborted;
    int profileWipeSkipped;
    if (nEPWipeMode == BATCH) {
        InProgress::currentStep = stepTitles;
        InProgress::totalSteps = totalTitlesToFix;
    } else {
        InProgress::currentStep = 0;
        InProgress::totalSteps = 1;
    }
    for (int i = 0; i<titlesCount; i++) {
        ...
        bool titleAlreadyBackedUp = false;
        ...
        if (dir != nullptr) {
            ...
            while ((data = readdir(dir)) != nullptr) {
                ...
                if (data->d_name[0] == '8') {
                    if (! profileExists(data->d_name)) {
                        stepProfiles++;
                        bool wipeInteractive = false;
                        if ( nEPWipeMode != BATCH) {
                            char message[1024];
                            snprintf(message,1024,LanguageUtils::gettext("[%d / %d ] WARNING\n\nTitle %s\nhas %s savedata for the non-existent profile '%s'\nThis savedata wll be ignored\n\nDo you want to wipe it?"),stepProfiles, totalProfilesToFix,titles[i].shortName,titles[i].isTitleOnUSB ? "USB":"NAND",data->d_name);
                            std::string Message(message);
                            std::string Choices = "\n\n\ue000 Wipe      \ue001 Skip      \ue002 Cancel All Process";
                            Message.append(Choices);
                            while (true) {
                                Button choice = promptMultipleChoice(ST_MULTIPLE_CHOICE,Message.c_str());
                                if (choice == PAD_BUTTON_B) {
                                    profileWipeSkipped++;
                                    skip = true;
                                    break;
                                }
                                if (choice == PAD_BUTTON_X) {
                                    closedir(dir);
                                    promptError(LanguageUtils::gettext("Wipe non-exsistent profile savedata process aborted\n"));
                                    return -1;
                                }
                                if (choice == PAD_BUTTON_A) {
                                    wipeInteractive = true;
                                    break;
                                }
                            }
                            if (skip)
                        ...
                        if ( nEPWipeMode == BATCH || wipeInteractive ) {                       
                            BackupSetList::saveBackupSetSubPath();   
                            BackupSetList::setBackupSetSubPathToRoot();
                            int slotb = getEmptySlot(&titles[i]);
                            if (slotb >=0) {
                                bool doBckp = true;   // BATCH or DECIDE_WIPE
                                if (  nEPWipeMode == DECIDE_ALL  && ! titleAlreadyBackedUp )
                                    doBckp = promptConfirm(ST_YES_NO, LanguageUtils::gettext("Backup current savedata first to next empty slot?"));
                                if ( doBckp) {
                                    if (! titleAlreadyBackedUp ) {
                                        if (nEPWipeMode == BATCH)
                                            InProgress::currentStep++;
                                        InProgress::titleName.assign(titles[i].shortName);
                                        if (backupSavedata(&titles[i], slotb, -1, true , LanguageUtils::gettext("pre-wipeNon-ExistentProfileSave backup")) == 0 ) {
                                            titleAlreadyBackedUp = true;
                                        } else {
                                            std::string title (titles[i].shortName);
                                            std::string fail (LanguageUtils::gettext("Backup Failed - Wipe aborted !!"));
                                            std::string message = title+"\n"+fail;
                                            promptError(message.c_str());
                                            profileWipeAborted++;
                                        }
                                    }
                                    if (titleAlreadyBackedUp) {
                                        std::string profilePath = srcPath+"/"+data->d_name; 
                                        promptError("removed: %s %s",titles[i].shortName,data->d_name);
                                        //removeFolderAndFlush(profilePath);
                                        profilesWiped++;
                                    }
                                }
                            } else {
                                promptError(LanguageUtils::gettext("%s\nCannot get an empty slot. Backup and wipe aborted."),titles[i].shortName);
                                profileWipeAborted++;
                            }
                            BackupSetList::restoreBackupSetSubPath();                      
                        }
...
int reviewTitlesWithNonExistentProfiles(Title* titles, int titlesCount) {
    ...
    std::vector<titlesNEProfiles> titlesAndNEProfiles;
    ...
    for (int i = 0; i<titlesCount; i++) {   
        ...
        if (dir != nullptr) {
            ...
            while ((data = readdir(dir)) != nullptr) {
                ...
                if (data->d_name[0] == '8') {
                    if (! profileExists(data->d_name)) {
                        nEProfiles.append(data->d_name).append(" ");                      
                    }
                }
            }
        }
        closedir(dir);
        if (nEProfiles.length() > 0)
            titlesAndNEProfiles.push_back({
                .index = i,
                .nEProfiles = nEProfiles
            });

        if (titlesAndNEProfiles.size()>12) {
            showTitlesAndNEProfiles(titles,titlesAndNEProfiles);
            titlesAndNEProfiles.clear();
        }
    }
    if (titlesAndNEProfiles.size()>0)
        showTitlesAndNEProfiles(titles,titlesAndNEProfiles);
    return 0;
}
...
void showTitlesAndNEProfiles(Title * titles, std::vector<titlesNEProfiles> & titlesAndNEProfiles) {
    ...
    DrawUtils::setFontColor(COLOR_TEXT);
    size_t line;
    for (line=0;line < titlesAndNEProfiles.size();line++) {
        int i = titlesAndNEProfiles[line].index;
        consolePrintPos(M_OFF+1, 2+line, "%s %s >> %s",
            titles[i].shortName,
            titles[i].isTitleOnUSB ? "[U]" : "[N]",
            titlesAndNEProfiles[line].nEProfiles.c_str());
        if (titles[i].iconBuf != nullptr)
            DrawUtils::drawTGA((M_OFF + 4) * 12 - 2, (line + 3) * 24, 0.18, titles[i].iconBuf);
    }
    DrawUtils::setFontColor(COLOR_INFO);
    consolePrintPos(-2, 4+line, LanguageUtils::gettext("\ue07c Next Page"));
    DrawUtils::endDraw();
