...
bool FSUtils::createFolder(const char *path) {
    ...
    while ((pos = strPath.find('/', pos + 1)) != std::string::npos) {
        ...
        if (mkdir(dir.c_str(), 0666) != 0 && errno != EEXIST) {
            ...
            StringUtils::splitStringWithNewLines(dir, multilinePath);
...
bool FSUtils::removeDir(const std::string &pPath) {
    ...
    while ((data = readdir(dir)) != nullptr) {
        ...
        if (data->d_type & DT_DIR) {
            ...
            if (unlink(origPath.c_str()) == -1) {
                std::string multilinePath;
                StringUtils::splitStringWithNewLines(origPath, multilinePath);
            ...
            if (unlink(tempPath.c_str()) == -1) {
                std::string multilinePath;
                StringUtils::splitStringWithNewLines(tempPath, multilinePath);
    ...
    if (closedir(dir) != 0) {
        std::string multilinePath;
        StringUtils::splitStringWithNewLines(pPath, multilinePath);
...
bool FSUtils::copyFile(const std::string &sPath, const std::string &initial_tPath, bool if_generate_FAT32_translation_file /*= false*/) {
    ...
    if (dest == nullptr) {
        if ((errno == EINVAL || errno == ENAMETOOLONG || errno == ENOENT) && if_generate_FAT32_translation_file) {
            ...
            if (Escape::constructEscapedSourceAndTargetPaths(sPath, initial_tPath, escaped_spath, escaped_tpath, only_bp_escaped_spath)) {
                ...
                if (dest != nullptr) {
                    ...
                    fclose(source);
                    std::string multilinePath;
                    StringUtils::splitStringWithNewLines(escaped_tpath, multilinePath);
        ...
        fclose(source);
        std::string multilinePath;
        StringUtils::splitStringWithNewLines(tPath, multilinePath);
        Console::promptError(LanguageUtils::gettext("Cannot open file for write\n\n%s\n%s"), multilinePath.c_str(), strerror(errno));
        return false;
    }
...
copy_file:
    ...
    if (!success) {
        if (readError > 0) {
            std::string multilinePath;
            StringUtils::splitStringWithNewLines("reading from " + sPath, multilinePath);
            Console::promptError(LanguageUtils::gettext("Read error\n\n%s\n\n%s"), strerror(readError), multilinePath.c_str());
        }
        if (writeError > 0) {
            std::string multilinePath;
            StringUtils::splitStringWithNewLines("copying to " + sPath, multilinePath);
